...
public class Q0008 {
	...
	public static void main(String[] args) {
		...
		//日付のフォーマットと解析を行うための設計図を使用して、sdfという名前で、新しく中身をデフォルトに設定された初期値に設定し、()内にSimpleDateFormatのルールに基づいて指定されたフォーマットパターンに関するデータと処理を持った実体を作成する。()内は2026年 07月 23日 19時 09分のような形式を指定する。
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy年 MM月 dd日 HH時 mm分");
		
		
		//javaに標準実装されている基本的なコードの設計図を使い、標準的な出力を指し示す変数に対して、()内に書かれた値を、表示して改行する処理。()内は、現在時刻の情報が入っているデータや処理を持った実体に、その中に入っている時刻を取得する処理を実行して時間を取得し、それを先ほど指定して作ったフォーマットパターンに関するデータや処理を持った実体を使って、実際に取得した現在時刻をフォーマットして文字列とし、その前に"現在 "と、後に"をお知らせします。"の文字列を連結し、それをひとつの文字列としている。
